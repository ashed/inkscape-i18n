...
градиентные и паттерновые заливки, редактирование узлов, экпорти и импорт
...
для этих объектов. Цель состоит в том, что, так как SVG является
стандартом, и его файлы это text/xml, возможно использовать их в 
значительном числе программ и для широкого круга применений.

B<Inkscape> использует SVG как родной формат документов, и имеет целью стать
наиболее совместимой программой для рисования SVG файлов, из доступных
...
для EPS. Для PNG, может использоваться вместе с параметром L<--export-use-hints>.
...
Фоновый цвет экспортируемого PNG.  Любой SVG поддерживаемый цвет, строкой
например "#ff007f" или "rgb(255, 0, 128)".  Если не установлено, то
будет использоваться цвет страницы устанавливаемый в Inkscape 
...
(если не указывается L<--export-ignore-filters> для подавленияs
...
Открыть указанный документ(ы). Строка параметров может быть опущена,
...
можно переопределить использую параметр L<--export-area> (только PNG)
...
Печатать документ(ы) на указанный принтер используя `lpr -P PRINTER'.
...
(0.0 - значит полная прозрачность, 1.0 - полная непрозрачность) или
...
селектор PDF версии, найденный в диалоге PDF-экспорта в GUI. 
...
документов LaTeX, где текст изображений - это typeset по LaTeX.  При экспорте
в PDF/PS/EPS формат, эта опция разделяет вывод в PDF/PS/EPS файл (напр. 
как при --export-pdf) и LaTeX файл. Текст не будет выводом в PDF/PS/EPS файл, 
но появится в файле LaTeX. Этот LaTeX файл включает PDF/PS/EPS. Ввод
(\input{image.tex}) LaTeX файла в LaTeX документ покажет изображение 
и весь текст будет typeset by LaTeX. Смотрите результирующий LaTeX
файл для дальнейшей информации.  Также смотрите GNUPlot `epslatex' вывод
терминала.

=item B<-T>, B<--export-text-to-path>

Конвертировать текстовые объекты в контуры при экспорте, где применимо (для PS, EPS, PDF и SVG экспорта).

=item B<--export-ignore-filters>

Экспорт фильтрованных объектов (напр. тех, которые с размытием) как векторов, игнорируя фильтры (для PS, EPS, и PDF экспорта).  Умолчально, все фильтрованные объекты растеризуются как L<--export-dpi> (умолчально 96 dpi), сохраняя видимость.
